...
//程序的帮助
//oracle不需要为查询语句创建专用的数据库连接(不会出现查询取结果集未完成前不能执行其他sql语句的情况）
//oracle不需要MAXPARAMS宏（绑定参数的最大数量）（直接定义该宏）
...
void _help(char *argv[])
{
  ...
  //使用rowid代替keyid，速度更快，详见oracle伪列
  printf("Sample:/project/tools1/bin/procctl 3600 /project/tools1/bin/deletetable_oracle /log/idc/deletetable_oracle_ZHOBTMIND1.log \"<connstr>sxixia/xixisbx@snorcl11g_144</connstr><tname>T_ZHOBTMIND1</tname><keycol>rowid</keycol><where>where ddatetime<sysdate-0.03</where><starttime>01,02,03,04,05,13</starttime><timeout>120</timeout><pname>deletetable_oracle_ZHOBTMIND1</pname>\"\n\n");
