...
//keyid字段的处理
//时间函数和时间格式
//如果sql的语句有问题，mysql的prepare()会返回错误，oracle不会。所以为了兼容性考虑，尽可能不判断prepare的返回值，execute时也会报错。
...
void _help()
{
  ...
  printf("Sample:/project/tools1/bin/procctl 3600 /project/tools1/bin/dminingoracle /log/idc/dminingoracle_ZHOBTCODE.log \"<connstr>sxixia/xixisbx@snorcl11g_144</connstr><charset>Simplified Chinese_China.AL32UTF8</charset><selectsql>select obtid,cityname,provname,lat,lon,height from T_ZHOBTCODE1</selectsql><fieldstr>obtid,cityname,provname,lat,lon,height</fieldstr><fieldlen>10,30,30,10,10,10</fieldlen><bfilename>ZHOBTCODE1</bfilename><efilename>HYCZ</efilename><outpath>/idcdata/dmindata</outpath><timeout>30</timeout><pname>dminingoracle_ZHOBTCODE1</pname>\"\n\n");
  printf("       /project/tools1/bin/procctl   30 /project/tools1/bin/dminingoracle /log/idc/dminingoracle_ZHOBTMIND1.log \"<connstr>sxixia/xixisbx@snorcl11g_144</connstr><charset>Simplified Chinese_China.AL32UTF8</charset><selectsql>select obtid,to_char(ddatetime,'yyyymmddhh24miss'),t,p,u,wd,wf,r,vis,keyid from T_ZHOBTMIND1 where keyid>:1 and ddatetime>sysdate-0.04</selectsql><fieldstr>obtid,ddatetime,t,p,u,wd,wf,r,vis,keyid</fieldstr><fieldlen>10,19,8,8,8,8,8,8,8,15</fieldlen><bfilename>ZHOBTMIND1</bfilename><efilename>HYCZ</efilename><outpath>/idcdata/dmindata</outpath><starttime></starttime><incfield>keyid</incfield><incfilename>/idcdata/dmining/dminingoracle_ZHOBTMIND1_HYCZ.list</incfilename><timeout>30</timeout><pname>dminingoracle_ZHOBTMIND1_HYCZ</pname><maxcount>1000</maxcount><connstr1>sxixia/xixisbx@snorcl11g_144</connstr1>\"\n\n");
...
bool _xmltoarg(char *strxmlbuffer)
{
  ...
  for (int ii=0;ii<CmdStr.CmdCount();ii++)
  {
    ...
    // if (ifieldlen[ii]>imaxfieldlen) ifieldlen[ii]=imaxfieldlen;   // 字段的长度不能超过MAXFIELDLEN。
