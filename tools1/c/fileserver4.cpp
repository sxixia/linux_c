...
bool RecvFile(const int sockfd,const char *filename,const char *mtime,int filesize);	//接收文件内容的函数
...
void RecvFilesMain()
{
 while(true)
 {
  ...
  {
   logfile.Write("RecvFilesMain(接收请求报文): TcpServer.Read() failed.\n"); return;
  }
  ...
  {
   strcpy(strsendbuffer,"ok");
  //logfile.Write("RecvFilesMain(): strsendbuffer=%s\n",strsendbuffer);
   if (TcpServer.Write(strsendbuffer)==false)
    {
    logfile.Write("RecvFilesMain(心跳): TcpServer.Write() failed.\n"); return;
    }
  }

  //处理上传文件的请求报文xml。
  //如果是上传文件的请求，报文会以<filename>开头
  if(strncpy(strrecvbuffer,"<filename>",10)==0)
  {
   //解析上传文件请求报文的xml。
   ...
   //客户端和服务端的文件的目录是不一致的，所以要先生成服务端的文件名
   //UpdateStr函数把文件名中的clientpath替换成srvpath，要小心第三个参数
   ...
   if (TcpServer.Write(strsendbuffer)==false)
    {
    logfile.Write("RecvFilesMain(接收文件结果): TcpServer.Write() failed.\n"); return;
    }
  }
 }
}
...
bool RecvFile(const int sockfd,const char *filename,const char *mtime,int filesize)
{
 ...
 while(true)
 {
  ...
  //计算已经接收的总字节数，如果已经接收完成，跳出循环
  totalbytes+=onread;
  if(totalbytes==filesize) break;
 }
 //关闭临时文件
 fclose(fp);
 //重置文件时间,将传输文件的时间改成文件原始的时间
