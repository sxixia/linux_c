...
{
 char colname[31];	// 列名
 char datatype[31];	// 列的数据类型，分为number，date，和char三大类
 int collen;		// 列的长度，number固定为20，date固定为19，char的长度由表的结构决定
...
public:
  ...
  char m_pkcols[301];			// 主键字段名列表，子字符串存放，中间用半角的逗号分隔

  void initdata();			// 成员表量初始化
